...
def test_pwa_identity_handoff():
    ...
    if "speakchain-shell-v30" not in sw or "telegram_auth_callback.html" not in sw:
...
def test_live_rooms_fail_closed_until_two_account_gate():
    ...
    if missing or "speakchain-shell-v30" not in sw:
...
def test_chainy_memory_controls_contract():
    ...
    if "speakchain-shell-v30" not in worker or "'./chainy_memory.js'" not in worker:
...
def test_chainy_interest_ui_contract():
    ...
    if "speakchain-shell-v30" not in worker or "'./chainy_interest.js'" not in worker:
...
def test_day1_onboarding_accessibility_contract():
    ...
    worker = (ROOT / "sw.js").read_text(encoding="utf-8")
    if "speakchain-shell-v30" not in worker or "'./index_v2.html'" not in worker:
...
def test_progress_security_accessibility_contract():
    ...
    worker = (ROOT / "sw.js").read_text(encoding="utf-8")
    if "speakchain-shell-v30" not in worker or "'./progress.html'" not in worker:
...
def test_player_bounded_seek_behavior():
    ...
    if "speakchain-shell-v30" not in worker or "'./player_seek.js'" not in worker:
