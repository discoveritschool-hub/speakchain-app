...
def test_friends_growth_layers():
    """Стара мережа/естафети не мають знову зникнути з вкладки «Друзі»."""
    section("Друзі — вигоди, мережа, естафета й кімната")
    if not SHELL.exists():
        fail("Не знайдено оболонку для перевірки вкладки «Друзі»")
        return
    src = SHELL.read_text(encoding="utf-8")
    markers = (
        'id="friends-network-count"',
        "Запросити друга",
        "+20% XP на 7 днів",
        "+15 XP і +1 ланка",
        "openFriendsLayer('relay')",
        "openFriendsLayer('challenge')",
        "openFriendsLayer('network')",
        "openFriendsLayer('room')",
        "async function openFriendsLayer(tab)",
    )
    missing = [marker for marker in markers if marker not in src]
    if missing:
        fail(f"Вкладка «Друзі»: бракує механік {missing}")
    else:
        ok("усі попередні механіки повернуті через компактні рівні")
