...
test('backend error keeps authoritative value and offers an explicit retry', async ({ appPage: page, context, scenario }) => {
  ...
  await expect(page.locator('#profile-settings-status')).toContainText('Не вдалося зберегти');
  await expect(page.locator('#profile-settings-status')).toHaveClass(/error/);
  await expect(page.locator('#profile-settings-save')).toHaveText('Спробувати ще раз');
  ...
  expect(profileMutations(scenario)).toHaveLength(2);
});
...
test('navigation and double save keep one active generation and ignore reordered completion', async ({ appPage: page, context, scenario }) => {
  scenario.profileMutationResponses.push(
    { delayMs: 4000, settings: { utc_offset: 5, notification_pref: 'evening' } },
    { delayMs: 10, settings: { utc_offset: 6, notification_pref: 'evening' } }
  );
  await context.addInitScript(() => {
